...
object Ch25_01_Feature_Engineering {

  val logger = Logger.getLogger(Ch25_01_Feature_Engineering.getClass)
  ...
  def main(args: Array[String]) {
    ...
    val fakeIntDF = spark.read.parquet(BASEPATH + "/data/simple-ml-integers")
    var simpleDF = spark.read.json(BASEPATH + "/data/simple-ml")
    val scaleDF = spark.read.parquet(BASEPATH + "/data/simple-ml-scaling")
    ...
    val tkn1 = new Tokenizer().setInputCol("Description")
    tkn1.transform(sales.select("Description")).show(false)
  ...
  """)
    ...
    val bucketer1 = new Bucketizer().setSplits(bucketBorders).setInputCol("id")
    bucketer1.transform(contDF).show()
    ...
    val lblIndxr1 = new StringIndexer().setInputCol("lab").setOutputCol("labelInd")
    val idxRes = lblIndxr1.fit(simpleDF).transform(simpleDF)
    idxRes.show()
    ...
    val tokenized2 = tkn2.transform(sales.select("Description"))
    tokenized2.show(false)
    ...
    val rt2 = new RegexTokenizer()
      .setInputCol("Description")
      .setOutputCol("DescOut")
      .setPattern(" ") // simplest expression
      .setToLowercase(true)
    rt2.transform(sales.select("Description")).show(false)
    ...
    val stops = new StopWordsRemover()
      ...
      .setInputCol("DescOut")
    stops.transform(tokenized2).show()
    ...
    unigram.transform(tokenized2.select("DescOut")).show(false)
    bigram.transform(tokenized2.select("DescOut")).show(false)
    ...
    val cv = new CountVectorizer()
      ...
      .setMinTF(1)
      .setMinDF(2)
    val fittedCV = cv.fit(tokenized2)
    fittedCV.transform(tokenized2).show(false)


    // COMMAND ----------

    // in Scala
    val tfIdfIn = tokenized2
